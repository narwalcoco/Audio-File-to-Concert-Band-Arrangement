...
fn pick_notes_crystal(data: &MatrixData, settings: &Settings, tempo: f32) -> Vec<NoteEvent> {
    ...
    // 8-frame Stability window (Ultra-smooth)
...
fn grade_precision(detected: &[NoteEvent], golden: &[GoldenNote], tempo: f32, settings: &Settings) -> f32 {
    ...
    // First note alignment (Zero Start)
    ...
    for g_note in golden {
        ...
        // Rather Be (Horn is Part 5). Usually written in F (shift is -7 from written to concert)
        let target_pitch = (g_note.pitch as i16 - 7 + settings.pitch_offset as i16) as u8;
    ...
    let mut penalty = 0.0;
    if detected.len() > (golden.len() * 2) {
        penalty = ((detected.len() as f32 - (golden.len() as f32 * 2.0)) / 50.0).min(1.0);
    }
    (recall * 0.90) - (penalty * 0.10)
}
...
fn main() -> Result<()> {
    ...
    println!("--- Super-Stable Trainer [{}] (1,000 Generations) ---", song);
    ...
    // SEED: Seeding with 'Rather Be' Best Settings (Session 17)
    let mut best_settings = Settings {
        onset_threshold: 0.05,
        frame_threshold: 0.09,
        min_midi: 21,
        max_midi: 108,
        alignment_offset: -0.106,
        pitch_offset: 8,
